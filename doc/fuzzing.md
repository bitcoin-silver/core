...
```sh
$ git clone --depth=1 https://github.com/bitcoinsilver-core/qa-assets
...
an instrumentalized libc++, then using it to build [BitcoinSilver dependencies
from source](../depends/README.md) and finally the BitcoinSilver fuzz binary
...
the qa-assets repo
(https://github.com/bitcoinsilver-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=bitcoinsilver-core%20status:open).

BitcoinSilver follows its [security disclosure policy](https://bitcoinsilvercore.org/en/security-advisories/),
