...
Additional less commonly used commands:
  ...
  chainstate [ARGS] Run bitcoin kernel chainstate util, equivalent to running 'bitcoinsilver-chainstate [ARGS]'.
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "bitcoinsilver-gui" : "bitcoinsilver-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "bitcoinsilver-node" : "bitcoinsilverd");
