...
static void SetupBitcoinSilverTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoinSilverTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The bitcoinsilver-tx tool is used for creating and modifying bitcoinsilver transactions.\n\n"
                "bitcoinsilver-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitcoinsilver transaction, or with \"-create [commands]\" to create a hex-encoded bitcoinsilver transaction.\n"
                "\n"
